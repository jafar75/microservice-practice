...
func (r *RedisRepo) Insert(ctx context.Context, order model.Order) error {
	...
	if _, err := txn.Exec(ctx); err != nil {
		return fmt.Errorf("failed to exec(commit) the txt: %w", err);
	}
...
func (r *RedisRepo) FindByID(ctx context.Context, id uint64) (model.Order, error) {
	...
	if errors.Is(err, redis.Nil) {
		...
		return model.Order{}, fmt.Errorf("get order with error: %w", err);
	}
...
func (r *RedisRepo) DeleteById(ctx context.Context, id uint64) error {
	...
	if errors.Is(err, redis.Nil) {
		...
		txn.Discard();
		return fmt.Errorf("get order with error: %w", err);
	}
...
func (r *RedisRepo) Update(ctx context.Context, order model.Order) error {
	...
	if errors.Is(err, redis.Nil) {
		...
		return fmt.Errorf("update order with error: %w", err);
	}
